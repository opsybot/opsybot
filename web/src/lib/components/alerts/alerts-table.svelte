<script lang="ts">
	...
	const columns: ColumnDef<Alert>[] = [
		...
		{ id: 'source', accessorKey: 'source' },
		{ id: 'service', accessorKey: 'service' },
		{
			id: 'label',
		...
		},
		{
			...
			filterFn: (row, id, value) => {
				const hours = String(value) === '24h' ? 24 : Number(String(value).replace('d', '')) * 24;
				return Date.parse(row.getValue(id) as string) >= now - hours * 3_600_000;
			}
		}
	];
				...
				{#if expanded.has(alert.id)}
					{#each alert.children as child (child.id)}
						<Table.Row class="hover:bg-transparent">
							...
							<Table.Cell class="bg-inset px-3 py-2" colspan={3}>
