<script lang="ts">
	...
	let grace = $state('2m');
	let policy = $state(ESCALATION_POLICIES[0]);
		...
		{:else}
			...
			>
				<div class="flex flex-col gap-3 p-6">
					...
					<div class="mt-1 flex flex-col gap-4">
						...
						<Field.Field class="gap-1.5 space-y-0">
							...
							<Select.Root type="single" name="policy" bind:value={policy}>
								<Select.Trigger>{policy}</Select.Trigger>
								<Select.Content>
									<Select.Group>
										{#each ESCALATION_POLICIES as entry (entry)}
											<Select.Item value={entry} label={entry}>{entry}</Select.Item>
										{/each}
									</Select.Group>
								</Select.Content>
							</Select.Root>
							<Field.FieldDescription class="text-subtle-foreground text-xs">
								A missed check-in pages whoever this policy names.
				...
				<div class="flex justify-end gap-2 border-t bg-black/20 px-6 py-4">
					...
					<Button type="submit" disabled={!name.trim()}>Create monitor</Button>
				</div>
			</form>
		{/if}
	</Dialog.Content>
